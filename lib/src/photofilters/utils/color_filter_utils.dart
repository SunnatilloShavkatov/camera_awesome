...
import "package:camera_awesome/src/photofilters/utils/utils.dart" as image_utils;
...
RGBA hueRotation(RGBA color, int degrees) {
  ...
  final num r = color.red;
  final num g = color.green;
  final num b = color.blue;
  return RGBA(
...
RGBA hueSaturation(RGBA color, num adj) {
  final List<num?> hsv = image_utils.rgbToHsv(color.red, color.green, color.blue);
  hsv[1] = (hsv[1] ?? 0) * adj;
  final List<num> rgb = image_utils.hsvToRgb(hsv[0]!, hsv[1]!, hsv[2]!);
  return RGBA(
...
RGBA colorOverlay(RGBA color, num red, num green, num blue, num scale) => RGBA(
...
// RGB Scale
RGBA rgbScale(RGBA color, num red, num green, num blue) => RGBA(
...
RGBA addictiveColor(RGBA color, int red, int green, int blue) => RGBA(
